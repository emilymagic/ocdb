...
typedef struct TransactionStateData
{
	...
	uint64 tupleSequence;		/* Sequence for new tuple */
...
GetCurrentFullTransactionId(void)
{
	...
	if (MemoryHeapActive())
	{
		...
		fullXid.value = MemoryHeapGetFullXid();
		return fullXid;
	}
...
AssignTransactionId(TransactionState s)
{
	...
	s->tupleSequence = FirstItemPointerSeq;

	if (myClusterId != 0)
		elog(ERROR, "We can only assign transaction id in catalog server.");
...
uint64
TransactionGetSequence(bool inc)
{
	if (!inc)
		return CurrentTransactionState->tupleSequence;
	else
		return ++CurrentTransactionState->tupleSequence;
}
