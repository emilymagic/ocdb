...
ClearMemoryHeapStorage(void)
{
	...
	BuildMemoryHeapStorage();
}

void
MemoryHeapDataSetCat(CdbCatalogNode *catalog)
{
	MemoryContext oldCtx;
	...
	MemoryHeapDataSetInternal(catalog, NULL);
	if (!IS_CATALOG_SERVER() && IS_QUERY_DISPATCHER())
	{
		if (!cdbCatAuxNode)
			cdbCatAuxNode = makeNode(CdbCatalogAuxNode);

		cdbCatAuxNode->catalog = catalog;
	}
...
void
MemoryHeapDataSetAux(AuxNode *aux)
{
	MemoryContext oldCtx;
	...
	MemoryHeapDataSetInternal(NULL, aux);
	if (!IS_CATALOG_SERVER() && IS_QUERY_DISPATCHER())
	{
		if (!cdbCatAuxNode)
			cdbCatAuxNode = makeNode(CdbCatalogAuxNode);
...
{
	MemoryHeapDataSetCat(catAux->catalog);
	MemoryHeapDataSetAux(catAux->aux);
}
