...
void
cc_send_modify_tabble(VisiNode *visiNode)
{
	CsQuery *csQuery;
	PGresult   *res;
	char 		*csQueryBuf;
...
GetCatalogTupleDesc(void)
{
	...
	{
		TupleDesc	tmp;
		MemoryContext oldcontext = MemoryContextSwitchTo(dataDispatchCtx);
...
void
cs_modify_table(VisiNode *visiNode)
{
	tile_insert_visi_cs(visiNode);
}
...
cs_get_catalog_from_sql(List *queryList, const char *sql, char *command)
{
	...
	if (dataDispatcher)
	{
		if (!dataDispatcher->hasPlOrTigger)
		{
			catAux = makeNode(CdbCatalogAuxNode);
